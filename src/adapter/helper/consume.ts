...
import { EventHandlerFunction, IEventManagerOptions } from '../../lib/interfaces';
...
export function consume(channel: amqp.Channel, queueName: string, listener: EventHandlerFunction, options: IEventManagerOptions): Promise<boolean> {
      ...
      channel.consume(queueName, message => {
        ...
        if (message) {
          try {
            ...
            if (message.fields.redelivered && hasReachedMaxNumberOfRetries(key, options.maxNumberOfMessagesRetries)) {
              LOGGER.warn(`Message with ${key} has been retried more than ${options.maxNumberOfMessagesRetries} times, flushing ...`);
              flushMessage({ message, channel, key, options });
              reject(new EventManagerError(`Message with ${key} has been retried more than ${options.maxNumberOfMessagesRetries} times`));
            } else {
              ...
              if (listenerInstance instanceof Promise) {
                listenerInstance
                  .then(ok => {
                    if (ok || typeof ok === 'undefined') {
                      ...
                      reject(new EventManagerError('Listener of event returned not true, so requeue message.'));
                    }
                  })
                  .catch(err => {
                    flushMessage({ message, channel, key, options });
                ...
                // Just set a warning
                LOGGER.warn(`Listener of queue ${queueName} is not a Promise, all messages will be acknowledged by default`, message);
                acknowledgeMessage({ message, channel, key });
                resolve(true);
              }
            }
          } catch (e) {
            LOGGER.debug(`Message unreadable, unable to parse JSON parsed on ${queueName}, flushing...`);
            flushMessage({
              message,
              channel,
...
function hasReachedMaxNumberOfRetries(key: string, maxNumberOfMessagesRetries: number) {
  return messagesCountRetries[key] && messagesCountRetries[key] >= maxNumberOfMessagesRetries;
}

...
describe("RabbitMQ Event Manager, consume Event max retries", () => {
  ...
  it(`Should nack a message that has been retried too many times`, done => {
    ...
    for (let i = 0; i < options.maxNumberOfMessagesRetries; i++) {
      promises.push(
        createPromiseForFirstCalls(message, channel, listener, options)
      );
    }
    Promise.all(promises)
      .then(() => {
        ...
        adapter
          ...
          .catch(() => {
            try {
              expect(channel.nack.callCount).to.equal(
                options.maxNumberOfMessagesRetries + 1
              );
              for (let i = 0; i < options.maxNumberOfMessagesRetries; i++) {
                expect(channel.nack.getCall(i).calledWith(message)).to.equal(
                  true,
                  `Call ${i} should have been called whithout parameters`
                );
              }
              expect(
                channel.nack.lastCall.calledWith(message, false, false)
              ).to.equal(
                true,
...
async function createPromiseForFirstCalls(
  message: any,
  channel: any,
  listener: any,
  options: any
